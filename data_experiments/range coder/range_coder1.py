...
Я об этом не знала а мобильного нету >:') Я психанула включила тот который 50 рэ но почемуто не работало Мужик раздал мне инет , п,Первое:
...
Найс, значит всё работает, только теперь надо как-то научиться комрессовать это ещё в смайлики что ли и будет чух А то что это такое почему так плохо получается - меня хафман почти обогнал. я 
я буду писать текст и параллельно улуччшать модель это прикольно хотя я просто тут писульки пишу свои а оно лучше язык понимает ислова часто используемые тоже вроде
та найс согласен что происходит в теории чем больше текста будет тем лучше будет сжатие да, соглашусь
кто отвечает, ты? я? может быть нет?
"""
...
rangeCoder.change_blend_alpha(1000)
